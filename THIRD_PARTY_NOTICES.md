...
| ripgrep 14.1.1 | checked-in target-specific upstream release binaries, redistributed as the AgentCore bundle `bin/rg[.exe]` runtime asset | `vendor/ripgrep/manifest.json` (https://github.com/BurntSushi/ripgrep, revision `4649aa9700`) | MIT OR Unlicense; MIT text retained at `vendor/ripgrep/LICENSE-MIT` and shipped with the bundle notice |
| TinyKG 0.2.0 | checked-in target-specific CLI binaries | `vendor/tinykg/manifest.json` (https://github.com/metask-ai/tinykg, source commit `a0544788aead`) | Apache-2.0; license retained at `vendor/tinykg/LICENSE` |
