...
- **file_lock**(`src/util/file_lock.zig`):`<path>.lock` O_EXCL 哨兵 + mtime 兜底陈旧检测 + **每条陈旧记录单赢家的接管**(防双持有):抢锁者先 O_EXCL 建这条记录专属的认领文件 `<path>.lock.claim.<指纹>.<代>`,持认领后确认锁文件仍是它观测到的那条陈旧记录才删除重建,别人刚抢到的新锁不会被挪走或删掉。
