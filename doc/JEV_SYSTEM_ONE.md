...
**只靠自动注入（付费，193 例 holdout，二进制 `810591a` / `fbe57c2`）**：上面的试点里模型会自己用
`KgRecall` 检索，自动注入选得好不好被主动检索掩盖了。这一组把 `KgRecall`/`KgContext`/`KgRemember`
从 provider schema 里拿掉（`tinykg_inject` / `tinykg_jev_inject` 臂），记忆库只放每轮一条的原子
记忆，模型只能看到召回门注入的至多 3 行（每行首行 ≤ 320 字节）。案例是 holdout 里全部可用精确
匹配判分的题（同样去掉偏好类与长答案），共 193 例，分三份并行。

| 臂 | 答对（精确匹配） | 注入记忆 token/例 | 每例成本 |
|---|---|---|---|
| `no_memory` | 0/193 | 0 | $0.005 |
| BM25 召回门 | 17/193 = 8.8% | 474 | $0.011 |
| **Jev 召回门** | **27/193 = 14.0%** | **406** | $0.010 |

- 配对：只有 Jev 答对 14 例、只有 BM25 答对 4 例，exact McNemar p = 0.031；自己的判分与项目
  `replay-memory` 在两份完整分片上逐行一致。
- 提升集中在 knowledge-update（5 → 9）与 single-session-user（6 → 13）；multi-session、
  temporal-reasoning、single-session-assistant 两臂持平（这些题要跨多条记忆计数或推算，三行
  注入本来就装不下）。
- 绝对值低是这个设定的上限：离线估计注入的三行里含金标答案的只有约 20%。所以这一组测的是
  “注入选得准不准”，不是“只靠注入能答对多少”。
- 注入块的固定说明（MANDATORY NEXT ACTION：先做一次精确 seed……）在这一组里指向了不存在的工具，
  两臂完全相同；实际一次越权调用都没有发生（每例 1 个请求、0 次工具调用）。
- 这组试点还暴露了一个产品 bug：注入行在 320 字节处截断时会切断多字节字符（中文等），provider
  收到的是 U+FFFD，注入回执与 provider 字节对不上，runner 按设计中止了两份分片。已修复
  （`2ca0c080`，三处同类截断一起改用 `util/utf8.prefixEnd`）；未跑完的 18 例用修复后的二进制
  `fbe57c2` 补齐。通过校验的行里注入字节本来就是合法 UTF-8，两个二进制对它们产出相同注入。
