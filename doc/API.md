...
| `METACODES_STREAM_IDLE_TIMEOUT_MS` | stream liveness: a request whose connection stays open but delivers no byte for this long (default 120000; 0 disables) is shut down by the client's monitor — before the response head it is retried as a transient network error, mid-stream the turn ends with `StreamStalled`. Nothing in a blocked socket read notices an abort flag; this is what turns a silent gateway or proxy into a bounded failure instead of a frozen session |
