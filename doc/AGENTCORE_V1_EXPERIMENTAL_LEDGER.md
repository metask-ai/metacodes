...
> RunContext 批次（`AGENTCORE_RUN_CONTEXT_DESIGN.md`）架构主体收敛，仍待消费端对
> 其 §1.3 两项（含并入的 A2）表态——A2 并入即意味着该批被 A2 扣押，不自称完全独立。
> 复冻前置条件（全部满足才形成候选）：
...
| A2 | Host 工具 FAILED/REJECTED 丢失错误详情，模型无法自纠 | `abi_v1.zig` AbiHostTool.execute 释放并忽略非 OK 内容 | **已并入 RunContext 批次**（同一回调面一次 breaking，见 RUN_CONTEXT_DESIGN §1.3），随该批消费端表态 |
...
- [ ] header 称 workspace_home 必须绝对路径，实现允许空值回退 root——二选一对齐；
- [ ] "不支持 background operations" → "不暴露 ABI 级异步 operation"（与 Bash 后台作业/BashOutput/KillShell 区分）；
- [ ] 明确 ABI 当前依赖 64 位指针布局；
- [ ] 明确禁止 C++ exception / longjmp 等非局部跳转跨越回调边界。
