...
fn nodeIdRange(nodes: []const segment_node_catalog.NodeInfoEntry) !segment_manifest.IdRange {
    if (nodes.len == 0) return error.InvalidRecord;
...
fn expectBundleOneHop(opened: *Opened, source_text: []const u8, source_id: core.NodeId, target_id: core.NodeId) !void {
    var source_ids = try opened.catalog.lookupExact(std.testing.allocator, .file, source_text, 8);
    defer source_ids.deinit(std.testing.allocator);
    try std.testing.expectEqualSlices(core.NodeId, &.{source_id}, source_ids.items);
    var neighbors = (try opened.edge_segment.neighborIterator(.forward, source_id, .defines)).?;
    const edge = (try neighbors.next()).?;
    try std.testing.expectEqual(source_id, edge.src);
    try std.testing.expectEqual(target_id, edge.dst);
    try std.testing.expectEqual(true, try opened.catalog.matchNode(target_id, .function, null));
    try std.testing.expectEqual(null, try neighbors.next());
}

test "segment bundle publishes and opens one-hop physical views" {
    ...
    try expectBundleOneHop(&opened, "src/main.zig", .fromInt(10), .fromInt(20));
}
...
test "segment bundle repeated publish advances generation and preserves pinned readers" {
    ...
    const next_nodes = [_]segment_node_catalog.NodeInfoEntry{
    ...
    try expectBundleOneHop(&first, "src/lib.zig", .fromInt(1), .fromInt(2));
    try expectBundleOneHop(&second, "src/app.zig", .fromInt(10), .fromInt(20));
}
...
test "segment bundle gc removes only unpinned generation data" {
    ...
    const gen1_nodes = [_]segment_node_catalog.NodeInfoEntry{
    ...
    const gen2_nodes = [_]segment_node_catalog.NodeInfoEntry{
    ...
    const gen3_nodes = [_]segment_node_catalog.NodeInfoEntry{
    ...
    try expectBundleOneHop(&first, "src/one.zig", .fromInt(1), .fromInt(2));
    first.deinit();
...
test "segment bundle publishes ordered edge streams" {
    ...
    try expectBundleOneHop(&opened, "src/a.zig", .fromInt(1), .fromInt(3));
}
