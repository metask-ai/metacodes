...
const SegmentHeader = struct {
    ...
    const encoded_len: usize = 129;
    const flag_u32_edge_records: u16 = 1 << 0;
    const flag_u32_vertex_records: u16 = 1 << 1;
    const flag_u16_relation_counts: u16 = 1 << 2;
    const flag_uniform_relation: u16 = 1 << 3;
    const flag_single_relation_per_vertex: u16 = 1 << 4;
    const flag_u24_node_u32_edge_records: u16 = 1 << 5;
    const flag_u24_node_u32_vertex_records: u16 = 1 << 6;
    const flag_u8_relation_counts: u16 = 1 << 7;
    const flag_uniform_relation_value_shift: u4 = 8;
    const flag_uniform_relation_value_mask: u16 = 0xff << flag_uniform_relation_value_shift;
    ...
    fn encode(self: SegmentHeader, out: *[encoded_len]u8) void {
    ...
    fn decode(bytes: []const u8) !SegmentHeader {
    ...
    fn validateShape(self: SegmentHeader) !void {
    ...
    fn hasU32EdgeRecords(self: SegmentHeader) bool {
        return (self.flags & flag_u32_edge_records) != 0;
    }

    fn hasU24NodeU32EdgeRecords(self: SegmentHeader) bool {
        return (self.flags & flag_u24_node_u32_edge_records) != 0;
    }

    fn hasU32VertexRecords(self: SegmentHeader) bool {
        return (self.flags & flag_u32_vertex_records) != 0;
    }

    fn hasU24NodeU32VertexRecords(self: SegmentHeader) bool {
        return (self.flags & flag_u24_node_u32_vertex_records) != 0;
    }

    fn hasU16RelationCounts(self: SegmentHeader) bool {
        return (self.flags & flag_u16_relation_counts) != 0;
    }

    fn hasU8RelationCounts(self: SegmentHeader) bool {
        return (self.flags & flag_u8_relation_counts) != 0;
    }

    fn hasUniformRelation(self: SegmentHeader) bool {
        return (self.flags & flag_uniform_relation) != 0;
    }

    fn hasSingleRelationPerVertex(self: SegmentHeader) bool {
        return (self.flags & flag_single_relation_per_vertex) != 0;
    }

    fn hasUniformSingleRelationEdgeCount(self: SegmentHeader) bool {
        return self.uniform_single_relation_edge_count != 0;
    }

    fn hasDerivedEdgeIds(self: SegmentHeader) bool {
        return self.derived_edge_id_step != 0;
    }

    fn hasDerivedVertexIds(self: SegmentHeader) bool {
        return self.derived_vertex_id_step != 0;
    }

    fn hasDerivedEdgeOtherNodes(self: SegmentHeader) bool {
        return self.derived_edge_other_node_step != 0;
    }

    fn uniformRelationValueFromFlags(flags: u16) u16 {
        return (flags & flag_uniform_relation_value_mask) >> flag_uniform_relation_value_shift;
    }

    fn expectedEdgeRecordLen(self: SegmentHeader) u16 {
    ...
    fn vertexRecordLen(self: SegmentHeader) u16 {
    ...
    fn relationRecordLen(self: SegmentHeader) u16 {
...
const DerivedEdgeIdShape = struct {
    base: u64,
    step: u32,
    split_index: u64 = 0,
    second_base: u64 = 0,
...
const DerivedVertexIdShape = struct {
    base: u64,
    step: u32,
    split_index: u64 = 0,
    second_base: u64 = 0,
...
const DerivedEdgeOtherNodeShape = struct {
    base: u64,
    step: u32,
    split_index: u64 = 0,
    second_base: u64 = 0,
...
fn derivedEdgeIdAt(header: SegmentHeader, index: u64) !u64 {
...
fn derivedEdgeIdShapesEqual(lhs: ?DerivedEdgeIdShape, rhs: ?DerivedEdgeIdShape) bool {
    if (lhs) |left| {
...
fn derivedVertexIdAt(header: SegmentHeader, index: u64) !u64 {
...
fn derivedVertexIdShapesEqual(lhs: ?DerivedVertexIdShape, rhs: ?DerivedVertexIdShape) bool {
    if (lhs) |left| {
        const right = rhs orelse return false;
        return left.base == right.base and
            left.step == right.step and
...
fn derivedEdgeOtherNodeAt(header: SegmentHeader, index: u64) !u64 {
...
fn derivedEdgeOtherNodeShapesEqual(lhs: ?DerivedEdgeOtherNodeShape, rhs: ?DerivedEdgeOtherNodeShape) bool {
    if (lhs) |left| {
        const right = rhs orelse return false;
        return left.base == right.base and
            left.step == right.step and
...
const VertexRecord = struct {
    ...
    const u32_encoded_len: usize = 16;
    const u24_node_u32_encoded_len: usize = 15;
    const single_relation_encoded_len: usize = 16;
    const single_relation_u32_encoded_len: usize = 8;
    const single_relation_u24_node_u32_encoded_len: usize = 7;
    const single_relation_rel_encoded_len: usize = 18;
    const single_relation_rel_u32_encoded_len: usize = 10;
    const single_relation_rel_u24_node_u32_encoded_len: usize = 9;
    const single_relation_uniform_edges_encoded_len: usize = 8;
    const single_relation_uniform_edges_u32_encoded_len: usize = 4;
    const single_relation_uniform_edges_u24_node_encoded_len: usize = 3;
    const single_relation_rel_uniform_edges_encoded_len: usize = 10;
    const single_relation_rel_uniform_edges_u32_encoded_len: usize = 6;
    const single_relation_rel_uniform_edges_u24_node_encoded_len: usize = 5;
    const u24_node_max: u64 = (1 << 24) - 1;

    fn hasU32Shape(self: VertexRecord) bool {
    ...
    fn hasU24NodeU32Shape(self: VertexRecord) bool {
    ...
    fn encode(self: VertexRecord, out: *[encoded_len]u8) void {
    ...
    fn encodeForHeaderAt(self: VertexRecord, header: SegmentHeader, index: u64, out: []u8) !void {
        try header.validateShape();
    ...
    fn decode(bytes: []const u8) !VertexRecord {
        if (bytes.len != encoded_len) return error.InvalidRecord;
        return .{
    ...
    fn decodeForHeaderAt(header: SegmentHeader, index: u64, bytes: []const u8) !VertexRecord {
        try header.validateShape();
...
const RelationRangeRecord = struct {
    rel: u16,
    edge_offset: u64,
    edge_count: u64,

    const encoded_len: usize = 10;
    const u16_encoded_len: usize = 4;
    const u8_encoded_len: usize = 3;
    const uniform_encoded_len: usize = 8;
    const uniform_u16_encoded_len: usize = 2;
    const uniform_u8_encoded_len: usize = 1;

    fn hasU16Count(self: RelationRangeRecord) bool {
        return self.edge_count <= std.math.maxInt(u16);
    }

    fn hasU8Count(self: RelationRangeRecord) bool {
        return self.edge_count <= std.math.maxInt(u8);
    }

    fn encode(self: RelationRangeRecord, out: *[encoded_len]u8) void {
        std.mem.writeInt(u16, out[0..2], self.rel, .little);
        std.mem.writeInt(u64, out[2..10], self.edge_count, .little);
    }

    fn encodeForHeader(self: RelationRangeRecord, header: SegmentHeader, out: []u8) !void {
        try header.validateShape();
    ...
    fn decode(bytes: []const u8) !RelationRangeRecord {
        if (bytes.len != encoded_len) return error.InvalidRecord;
        return .{
    ...
    fn decodeForHeader(header: SegmentHeader, bytes: []const u8) !RelationRangeRecord {
        try header.validateShape();
...
const StoredEdgeRecord = struct {
    other_node: u64,
    edge_id: u64,

    const encoded_len: usize = 16;
    const u32_encoded_len: usize = 8;
    const u24_node_u32_edge_encoded_len: usize = 7;
    const node_only_encoded_len: usize = 8;
    const u32_node_only_encoded_len: usize = 4;
    const u24_node_only_encoded_len: usize = 3;
    const u24_node_max: u64 = (1 << 24) - 1;

    fn fromEdge(edge: EdgeRecord, direction: Direction) StoredEdgeRecord {
        return .{
    ...
    fn toEdgeInRelation(self: StoredEdgeRecord, direction: Direction, owner_node: u64, relation_rel: u16) !EdgeRecord {
    ...
    fn decode(bytes: []const u8) !StoredEdgeRecord {
        if (bytes.len != encoded_len) return error.InvalidRecord;
        return .{
    ...
    fn decodeForHeaderAt(header: SegmentHeader, index: u64, bytes: []const u8) !StoredEdgeRecord {
        try header.validateShape();
...
fn deriveEdgeCountFromFileSize(file_size: u64, declared_edge_count: u64, vertex_count: u64, relation_count: u64, vertex_record_len: u16, relation_record_len: u16, edge_record_len: u16) !u64 {
...
fn fileSizeFor(vertex_count: u64, relation_count: u64, edge_count: u64) !u64 {
    return fileSizeForRecordLen(vertex_count, relation_count, edge_count, StoredEdgeRecord.encoded_len);
}

fn fileSizeForHeader(header: SegmentHeader) !u64 {
...
fn fileSizeForRecordLen(vertex_count: u64, relation_count: u64, edge_count: u64, edge_record_len: u16) !u64 {
...
fn fileSizeForRecordLens(vertex_count: u64, relation_count: u64, edge_count: u64, vertex_record_len: u16, relation_record_len: u16, edge_record_len: u16) !u64 {
    const vertex_bytes = std.math.mul(u64, vertex_count, vertex_record_len) catch return error.RecordTooLarge;
    const relation_bytes = std.math.mul(u64, relation_count, relation_record_len) catch return error.RecordTooLarge;
...
fn vertexRecordOffsetForHeader(header: SegmentHeader, index: u64) !u64 {
...
fn relationRecordOffset(header: SegmentHeader, index: u64) !u64 {
...
fn relationBaseOffsetForHeader(header: SegmentHeader) !u64 {
...
fn edgeBaseOffsetForHeader(header: SegmentHeader) !u64 {
    const vertex_bytes = std.math.mul(u64, header.vertex_count, header.vertexRecordLen()) catch return error.RecordTooLarge;
...
fn storedEdgeRangeOffset(edge_base: u64, edge_offset: u64, edge_record_len: usize) !u64 {
...
fn vertexEdgeOffsetFieldLen(header: SegmentHeader) usize {
    std.debug.assert(!header.hasUniformSingleRelationEdgeCount());
...
fn edgeOtherNodeForDirection(edge: EdgeRecord, direction: Direction) u64 {
    return switch (direction) {
...
fn relFromInt(value: u16) ?core.RelKind {
    if (value >= schema.max_relation_types) return null;
