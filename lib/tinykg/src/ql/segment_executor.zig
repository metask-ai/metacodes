...
pub const SegmentNodeCatalog = struct {
    ...
    pub fn validate(self: SegmentNodeCatalog) !void {
        ...
        var i: usize = 1;
        while (i < self.exact_texts.len) : (i += 1) {
            if (compareExactTextEntry(self.exact_texts[i - 1], self.exact_texts[i]) != .lt) return error.InvalidRecord;
        }
        i = 1;
        while (i < self.nodes_by_id.len) : (i += 1) {
            if (self.nodes_by_id[i - 1].id.toInt() >= self.nodes_by_id[i].id.toInt()) return error.InvalidRecord;
        }
        for (self.exact_texts) |entry| {
            const matches = (try self.matchNode(entry.id, entry.kind, entry.text)) orelse return error.InvalidRecord;
            if (!matches) return error.InvalidRecord;
