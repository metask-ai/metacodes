...
class MemoryAgentRuntimeContractTest(unittest.TestCase):
    ...
    def test_execution_episode_consolidation_is_bounded_private_and_content_addressed(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            write_text_lf(episode, "tampered episode\n", encoding="utf-8")
            episode.chmod(0o600)
    ...
    def test_checked_in_pilot_v15_uses_structural_grader_and_latent_protocol(self):
        pilot = ROOT / "evals/memory/pilots/procedural-glm52-v15"
        contract = json.loads((pilot / "pilot-contract.json").read_text(encoding="utf-8"))
        attempt = json.loads(
            (pilot / "attempt-001-observation.json").read_text(encoding="utf-8")
        )
        ...
        offline_prompts = [
            case["prompt"]
            for case in source["families"][0]["cases"]
            if case["split"] == "offline"
        ]
        self.assertEqual(len(offline_prompts), 2)
        self.assertTrue(all("REC3_<UPPER_SNAKE_NAME>" not in prompt for prompt in offline_prompts))
        self.assertTrue(all("snapshot > cross_check > seal" not in prompt for prompt in offline_prompts))
        check_kinds = {
            check["kind"]
            for case in validators["cases"]
            for check in case["validator"]["checks"]
        }
        self.assertEqual(check_kinds, {"json_equals", "python_assignment_equals"})
        self.assertEqual(manifest["execution"], execution)
        self.assertEqual(manifest["manifest_id"], "coding-intent-families-2026080820-1")
        self.assertEqual(contract["pilot_id"], "procedural-glm52-v15")
        self.assertEqual(contract["generation"]["split_seed"], 2026080820)
        self.assertEqual(contract["harness"]["revision"], execution["harness_revision"])
        self.assertEqual(
            contract["harness"]["git_commit"],
            "7462c822817f8b61c1b18a164689fca76e8031a0",
        )
        self.assertIn(
            "without executing candidate code",
            contract["paid_execution_requirements"]["semantic_grader"],
        )
        self.assertIn(
            "cannot launder malformed explicit plans",
            contract["paid_execution_requirements"]["query_plan_reporting"],
        )
        predecessor = contract["predecessor_attempts"]
        self.assertEqual(
            [(item["pilot_id"], item["status"]) for item in predecessor],
            [("procedural-glm52-v14", "completed-but-benchmark-uninformative")],
        )
        rows = [
            [item["sequence"], item["case_id"], item["trial"], item["arm"]]
            for item in manifest["schedule"]
        ...
        budget = contract["budget_authority"]
        self.assertEqual(budget["prior_conservative_cost_usd"], 10.4777678)
        self.assertEqual(budget["prior_conservative_metered_tokens"], 6711634)
        self.assertLessEqual(
            budget["prior_conservative_cost_usd"] + budget["max_total_cost_usd"],
            budget["user_authorization_max_cost_usd"],
        )
        self.assertTrue(contract["current_phase"]["paid_rollouts_authorized"])
        self.assertFalse(contract["current_phase"]["quality_evidence"])
        self.assertEqual(attempt["outcome"]["status"], "completed-pilot-signal")
        self.assertTrue(attempt["outcome"]["canonical_runtime_receipt_published"])
        self.assertFalse(attempt["outcome"]["quality_evidence"])
        self.assertEqual(attempt["outcome"]["committed_rollout_transactions"], 9)
        self.assertEqual(attempt["outcome"]["uncertain_authorized_transactions"], 0)
        self.assertTrue(attempt["evaluator_validation"]["all_rows_ready"])
        self.assertFalse(attempt["evaluator_validation"]["surface_form_failure_observed"])
        self.assertEqual(attempt["pilot_signal"]["no_memory"]["offline_successes"], 0)
        self.assertEqual(attempt["pilot_signal"]["markdown_memory"]["offline_successes"], 2)
        self.assertEqual(attempt["pilot_signal"]["tinykg_lexical"]["offline_successes"], 2)
        self.assertEqual(attempt["query_plan_diagnostic"]["invalid"], 0)
        self.assertEqual(attempt["query_plan_diagnostic"]["host_recall_satisfied"], 2)
        self.assertEqual(
            sum(item["provider_http_requests"] for item in attempt["rollouts"]),
            attempt["outcome"]["provider_http_requests"],
        )
    ...
    def test_checked_in_pilot_v16_freezes_multi_family_balanced_replication(self):
        ...
        self.assertEqual(len(offline_prompts), 8)
        hidden_terms = (
            "CAP4_",
            "LEASE4_",
            "EVID4_",
        ...
        self.assertTrue(
            all(term not in prompt for prompt in offline_prompts for term in hidden_terms)
        )
        check_kinds = {
            check["kind"]
            for case in validators["cases"]
        ...
        self.assertEqual(check_kinds, {"python_assignment_equals"})
        self.assertEqual(manifest["execution"], execution)
        self.assertEqual(manifest["manifest_id"], "coding-intent-families-2026080816-4")
        ...
        self.assertIn("McNemar", contract["analysis_plan"]["primary_test"])
        self.assertIn("10000", contract["analysis_plan"]["uncertainty"])
        self.assertFalse(contract["current_phase"]["quality_evidence"])
        self.assertTrue(contract["current_phase"]["paid_rollouts_authorized"])
        self.assertEqual(
            [
                (item["pilot_id"], item["status"])
        ...
        self.assertGreaterEqual(
            budget["max_total_cost_usd"],
            budget["max_rollout_cost_usd"] * len(manifest["schedule"]),
        )
        self.assertGreaterEqual(
            budget["max_total_metered_tokens"],
            budget["max_rollout_metered_tokens"] * len(manifest["schedule"]),
        )
        self.assertLessEqual(
            budget["prior_conservative_cost_usd"] + budget["max_total_cost_usd"],
            budget["user_authorization_max_cost_usd"],
    ...
    def test_checked_in_pilot_v18_has_fresh_paid_identity_and_balanced_schedule(self):
        ...
        self.assertEqual(
            attempt["outcome"]["status"], "halted-provider-authentication-rejected"
        )
        self.assertEqual(attempt["outcome"]["completed_rollout_transactions"], 0)
        self.assertEqual(attempt["outcome"]["uncertain_authorized_transactions"], 1)
        self.assertEqual(
            attempt["budget_journal"]["transaction_states"],
    ...
    def test_failed_paid_run_persists_budget_and_artifact_checkpoint(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            write_text_lf(rollout_dir / "result.json", "{}\n", encoding="utf-8")
    ...
    def test_paid_pilot_main_checkpoints_a_committed_mid_schedule_failure(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            write_text_lf(manifest_path, stable_json(manifest) + "\n", encoding="utf-8")
            ...
            tinykg.chmod(0o700)
            auth = root / "auth.json"
            write_text_lf(auth, stable_json({"api_key": "test-only-secret"}) + "\n", encoding="utf-8")
            auth.chmod(0o600)
            run_dir = root / "run"
            ...
            def fail_after_commit(**kwargs):
                ...
                write_text_lf(rollout / "provider-result.json", "{}\n", encoding="utf-8")
    ...
    def test_production_secret_scan_rejects_artifact_and_pending_receipt(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            write_text_lf(root / "stdout.ndjson", "safe\n", encoding="utf-8")
            _assert_production_secret_absent(
                root,
                secret,
    ...
    def test_production_auth_rejects_parent_environment_and_reads_private_file(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            write_text_lf(auth, '{"api_key":"private-file-key"}\n', encoding="utf-8")
            auth.chmod(0o600)
    ...
    def test_production_seatbelt_denies_host_sibling_and_process_info(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            write_text_lf(host, "host-secret\n", encoding="utf-8")
            write_text_lf(sibling, "sibling-secret\n", encoding="utf-8")
            ...
            write_text_lf(memory_index, "- [Procedure](procedure.md) -- durable pattern\n", encoding="utf-8")
            ...
            write_text_lf(store_manifest, '{"revision":1}\n', encoding="utf-8")
    ...
    def _materialize_artifacts(self, root, receipt):
        for rollout in receipt["rollouts"]:
            ...
            for path_key, hash_key in (
                ...
                write_text_lf(path, f"{path_key}:{rollout['sequence']}\n", encoding="utf-8")
    ...
    def _materialize_v3_artifacts(self, root, manifest, observations, receipt):
        ...
        for source in receipt["runner_sources"]:
            ...
            path.parent.mkdir(parents=True, exist_ok=True)
            write_text_lf(path, f"runner-source:{source['module']}\n", encoding="utf-8")
        ...
        for rollout in receipt["rollouts"]:
            ...
            for path_key, hash_key in (
                ...
                path = root / paths[path_key]
                path.parent.mkdir(parents=True, exist_ok=True)
                write_text_lf(path, file_payloads[path_key], encoding="utf-8")
                rollout[hash_key] = hashlib.sha256(path.read_bytes()).hexdigest()
            ...
            for request_id in range(1, rollout["provider_requests"] + 1):
                ...
                write_text_lf(
                    cassette / f"req-{request_id:03d}.json",
    ...
    def _v7(self, root):
        ...
        for rollout, observation in zip(receipt["rollouts"], observations):
            ...
            write_text_lf(first_request, stable_json(body) + "\n", encoding="utf-8")
            for request_path in sorted(cassette.glob("req-*.json")):
            ...
            if memory_root is not None:
                ...
                if case["split"] == "offline" and not index_path.exists():
                    write_text_lf(index_path, "# Durable memory\n", encoding="utf-8")
                    index_path.chmod(0o600)
                    ...
                    write_text_lf(first_request, stable_json(first) + "\n", encoding="utf-8")
    ...
    def _v4(self, root):
        ...
        for rollout in receipt["rollouts"]:
            ...
            if runtime_arm == "tinykg":
                ...
                write_text_lf(memory_root / "MEMORY.md", "# Integrated memory\n", encoding="utf-8")
    ...
    def _v5(self, root):
        ...
        write_text_lf(runner_path, "production budget journal source\n", encoding="utf-8")
        ...
        for rollout in receipt["rollouts"]:
            ...
            write_text_lf(probe_path, stable_json(probe) + "\n", encoding="utf-8")
            rollout["sandbox"] = {
                "backend": PRODUCTION_SANDBOX_BACKEND,
    ...
    def test_v3_reopens_source_memory_and_cassette_semantics(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            write_text_lf(runner, "tampered runner\n", encoding="utf-8")
    ...
    def test_v3_query_plan_source_binding_requires_replayable_sidecars(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            source_path = root / "runner-sources" / f"{module}.py"
            write_text_lf(source_path, "query-plan analyzer source\n", encoding="utf-8")
            receipt["runner_sources"].append(
                {
                    "module": module,
                    "path": f"runner-sources/{module}.py",
    ...
    def test_v5_receipt_binds_durable_budget_transactions_and_checkpoint(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            write_text_lf(profile_path, "tampered sandbox profile\n", encoding="utf-8")
            ...
            write_text_lf(probe_path, stable_json(failed_probe) + "\n", encoding="utf-8")
            ...
            write_text_lf(probe_path, stable_json(forged_tinykg_probe) + "\n", encoding="utf-8")
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            write_text_lf(memory / "late.md", "tampered\n", encoding="utf-8")
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            write_text_lf(request, stable_json(body) + "\n", encoding="utf-8")
    ...
    def test_v7_receipt_binds_host_recall_and_consolidation_artifacts(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            write_text_lf(episode, "tampered\n", encoding="utf-8")
            episode.chmod(0o600)
            with self.assertRaisesRegex(
                ValidationError,
    ...
    def test_v4_receipt_binds_production_budget_treatment_and_unknown_bill(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            runner = root / receipt["runner_sources"][0]["path"]
            write_text_lf(runner, "tampered production runner\n", encoding="utf-8")
            with self.assertRaisesRegex(ValidationError, "runtime source mismatch"):
                validate_runtime_artifacts(receipt, root)
