...
pub fn decode(allocator: std.mem.Allocator, encoded: []const u8) Error!?DecodedState {
    ...
    if (encoded.len < header_bytes or encoded.len > MAX_STATE_BYTES or
        !std.mem.eql(u8, encoded[0..magic.len], magic) or
        std.mem.readInt(u16, encoded[8..10], .little) != STATE_REVISION or
