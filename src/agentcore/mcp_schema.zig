...
const JsonAdmission = json.Admission;
...
fn admitJsonStructure(
    ...
    return json.admit(allocator, encoded, .{
...
test "argument validation enforces only bounded JSON object envelope" {
    ...
    try std.testing.expect(validateArguments(
        std.testing.allocator,
        "{\"escaped\\u004bey\":1,\"line\":\"one\\ntwo\",\"quote\":\"\\\"\",\"slash\":\"\\\\\",\"unicode\":\"\\u0041\"}",
        .{},
    ) == .valid);
    const invalid_escape = validateArguments(std.testing.allocator, "{\"value\":\"\\q\"}", .{});
    try std.testing.expect(invalid_escape == .invalid);
    try std.testing.expectEqual(ValidationIssue.invalid_json, invalid_escape.invalid);
...
test "schema admission accepts escaped JSON strings" {
    const escaped = testTool(
        "{\"type\":\"object\",\"properties\":{\"code\":{\"type\":\"string\",\"pattern\":\"^\\\\d{4}$\"}}}",
        null,
    );
    var admission = try prepareTool(std.testing.allocator, "mcp__weather", &escaped, .{});
    defer if (admission == .available) admission.available.deinit();
    try std.testing.expect(admission == .available);
}
