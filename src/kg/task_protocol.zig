...
pub const SYSTEM_RULES =
    ...
    \\- EXPERIENCE: a successful claim, or TaskGet recovery of a claimed task, may also return an `experience_packet` built from verified completed tasks and their execution-grounded ontology edges. Inspect it before any work. It is one bounded exact lexical probe, not proof of completeness; preserve tentative/confirmed state, follow evidence ids when relevant, and verify current reality before reusing prior behavior.
    \\- LEXICAL EXPANSION: TinyKG has no vectors. If the exact experience packet is insufficient, before work actively infer 2-4 separate compact semantic variants from this task (synonym/paraphrase, Chinese/English alias, mechanism, symptom, desired outcome, or nearby implementation term), call KgRecall once per useful variant, deduplicate node ids, and stop when evidence is sufficient. Never combine the whole neighborhood into one keyword bag.
