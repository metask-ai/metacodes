...
pub const SYSTEM_RULES =
    ...
    \\- Step 3E — ENUMERATION REQUIRES COVERAGE: for count/cardinality, exhaustive-list, all/every, or negative/absence questions set intent=`enumeration`. One positive hit proves existence, never completeness. Unless the seed returns an authoritative aggregate, submit a 2-4 member semantic batch; a successful v3 receipt proves every member ran. Batch execution is necessary, not sufficient: when the governed run has candidates and KgContext is available, call it on at least one recalled node and verify that merged hits and graph evidence cover the requested scope before concluding a count, exhaustive list, or absence. The host rejects premature final answers until the available obligations commit; a zero-candidate batch does not create an impossible KgContext obligation.
...
pub const ENUMERATION_CONTEXT_REMINDER =
    "[lexical-evidence-obligation] The enumeration batch committed and returned candidates, but no recalled node has been verified through KgContext. Before a final count/list/absence conclusion, call KgContext on the best node_id from the governed recall result and inspect its authoritative text, graph scope, evidence, freshness, and supersession signals.";

pub const ENUMERATION_CONTEXT_REPAIR =
    "[lexical-evidence-rejected-final] Your proposed final answer was rejected because the enumeration candidates remain unverified. Do not repeat KgRecall. Call KgContext now on a node_id returned by the governed recall batch, inspect its graph and governance receipt, then produce the final answer.";
