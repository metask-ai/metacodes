...
pub const SYSTEM_RULES =
    ...
    \\- Step 3E — ENUMERATION REQUIRES COVERAGE: for count/cardinality, exhaustive-list, all/every, or negative/absence questions set intent=`enumeration`. One positive hit proves existence, never completeness. Unless the seed returns an authoritative aggregate, submit a 2-4 member semantic batch; a successful v3 receipt proves every member ran. Batch execution is necessary, not sufficient: when the governed batch has candidates, the host deterministically runs one real KgContext re-observation and attaches it as auto_context in the same tool result. Inspect that authoritative text, graph scope, evidence, freshness, and supersession receipt before concluding a count, exhaustive list, or absence. Do not issue another KgContext unless auto_context explicitly signals contradiction, supersession, truncation, or missing evidence. The host rejects premature final answers until the available obligations commit; a zero-candidate batch does not create an impossible KgContext obligation.
    \\- Step 4 — MERGE AND VERIFY: semantically judge hits. Deduplicate candidates by node_id across every call. Memory is a candidate, not a current fact. Prefer exact aliases and authoritative nodes, but never treat score or wording overlap as correctness. Use a valid host-generated auto_context when present; otherwise call KgContext on the best seed to read its authoritative node text and bounded graph neighborhood.
...
pub const TOOL_DESCRIPTION =
    ...
    "Deduplicate node_id values across calls. A non-empty enumeration batch attaches one host-generated auto_context; inspect it instead of repeating KgContext unless it reports incomplete or conflicting evidence. Otherwise use KgContext to read authoritative node text and structured evidence/freshness/supersession signals. Memory hits are candidates, never automatically current facts. Hits carrying a \"source\" field come from a memory markdown file; update that file instead of KgRemembering a duplicate.";
...
pub const AUTO_RECALL_NEXT_ACTION =
    "强制下一步 / MANDATORY NEXT ACTION: 先检查上面的命中。若有 canonical alias/代码符号，先用一个无 type 的精确 seed。否则先做一个精确/高精度 seed；仅在不足时一次声明 2-4 个分离的语义变体，由宿主整批执行并按 node_id 合并。枚举批次会附带宿主生成的 auto_context；其它场景再用 KgContext 读取最佳节点及相连证据。";

/// 每次 KgRecall 结果都携带：把下一步决策放在使用时点，而非只依赖 system prompt。
pub const RESULT_GUIDANCE =
    "Semantically judge the merged lexical candidates. Each hit declares text_total_bytes/text_truncated under a UTF-8 head/tail excerpt policy; use the attached authoritative auto_context or KgContext when the omitted middle could affect the answer. content_ref=exposed_elsewhere_in_run points to the same node body already exposed in this run, not missing evidence. A v3 all_variants_executed receipt proves declared probes ran, not that the answer is complete. execution=host_seed_shape_rewrite means only an accidental exact prefix ran as a seed; every following semantic declaration was explicitly left unexecuted, so issue one fresh 2-4 member non-exact batch if the seed is insufficient. For a real semantic batch, do not issue its members separately or combine them into a keyword bag. An enumeration batch with candidates includes host-generated auto_context from the real KgContext path; inspect it and do not repeat KgContext unless it reports contradiction, supersession, truncation, or missing evidence. Never promote a hit to a current fact without evidence/freshness/supersession checks; lexical misses do not prove absence.";
