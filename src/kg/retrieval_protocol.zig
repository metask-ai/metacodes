...
pub const SYSTEM_RULES =
    ...
    \\- Step 2C — RECORD THE PLAN: in normal agent use, attach `lexical_plan` with schema_version lexical-query-plan-v2 to every KgRecall. A seed plan has exactly one exact/alias variant. For a semantic call, submit either the one non-exact probe chosen now or a fixed 2-4 member batch and select one variant_index; if you use a batch, reuse it unchanged for its later members. The run-scoped host ledger exclusively owns seen state and returns a stable per-plan plan_sha256 plus host-measured new/repeated hit counts across all v2 KgRecall calls. Query-only and v1 calls remain compatibility paths, not the governed default.
    \\- Step 3 — BOUNDED SEMANTIC NEIGHBORHOOD: only if the seed is insufficient, infer up to four separate compact semantic probes for this specific intent. Prefer a useful small set over filling a quota. The host permits at most four v2 semantic-expansion calls in the run. Each KgRecall executes ONE selected probe, not a combined keyword bag. Choose useful dimensions rather than mechanically using all of them: (a) synonym/paraphrase; (b) Chinese/English form, abbreviation, old/new name, or code identifier; (c) mechanism, symptom, desired outcome, or nearby implementation term; (d) one plausible broader or narrower concept. Run probes in stages and reassess after each result.
...
pub const TOOL_DESCRIPTION =
    ...
    "Use when the user refers to prior decisions/context or when continuing cross-session work. Retrieval is LEXICAL BM25: TinyKG has no embeddings and computes no vector distance. Start exact, then if needed issue 1-4 separate compact semantic variants and judge them yourself; never combine the whole neighborhood into one keyword bag. " ++
    "Attach lexical_plan (lexical-query-plan-v2) so the chosen intent, probe or small batch, selected index, host-owned seen state, and information gain are replayable. " ++
...
pub const QUERY_DESCRIPTION =
    "FIRST inspect automatic recall. If it exposed an exact canonical alias/code symbol, this query MUST contain ONLY that exact term plus user-requested field names; OMIT type. Otherwise begin with one compact exact/high-precision query using user wording. If that is insufficient, execute at most four separate semantic probes: a synonym/paraphrase; Chinese/English, abbreviation, old/new name, or code identifier; a mechanism/symptom/outcome/nearby implementation term; or one plausible broader or narrower concept. A call may declare its one selected probe or a fixed 2-4 member batch; never combine probes into one keyword bag. When lexical_plan is present, query must exactly equal variants[variant_index].text. Reassess after each call, deduplicate node_id values across calls, and stop when authoritative evidence is sufficient. A lexical miss does not prove absence.";

pub const PLAN_DESCRIPTION =
    "Governed, machine-observable lexical-query-plan-v2 artifact. Use it in normal agent calls. Seed: one exact/alias variant, index 0, no type filter. Expansion: after an insufficient seed, declare either the one typed non-exact probe selected now or a fixed 2-4 member batch; a batch stays identical while variant_index advances. The host permits at most four v2 semantic-expansion calls per run. Submit only the fields in this schema; the run-scoped host ledger exclusively owns seen state and measures new vs repeated hits across all v2 KgRecall calls. This records the LLM's semantic choices; TinyKG still performs lexical BM25 only.";
