...
fn sameTaskOutcomeNote(allocator: std.mem.Allocator, kg: *client_mod.KgClient) ?[]u8 {
    ...
    return std.fmt.allocPrint(
        ...
        "<system-reminder>\n# 本任务上一次尝试的判定结局(host 声明,确定性注入)\n" ++
            ...
            "然后在工作区构造并运行一个能复现该失败面的检查,再动手改。" ++
            "测试名本身是规格陈述——名字里的模块名、文件名、算法名、行为词都当作显式要求;" ++
            "当它与仓库既有惯例冲突时,以测试名为准。" ++
            "若你的实现思路与上一次尝试相同,判定结果也将相同:对每个失败点先列出至少两种可能的规格解释," ++
            "用工作区证据裁决后再实现。修复失败面时不要挤占已通过的行为——" ++
            "收尾前把你的整套检查重跑一遍,确认没有为修新账破旧账。失败名不要照抄进代码,也不要因此跳过验证。\n" ++
            "</system-reminder>\n",
        .{body},
    ) catch null;
}
