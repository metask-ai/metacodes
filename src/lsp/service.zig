...
test "Service e2e: 真 zls 报类型错误的 delta 诊断(需装 zls)" {
    ...
    const base = a.dupe(u8, @import("../util/fs.zig").testing.perPidDir(&base_buf, "cc-zig-lsp-zls")) catch return;
    defer a.free(base);
    mkdirZ(base);
...
test "Service e2e: 真 zls documentSymbol 抽 struct/function 符号(需装 zls)" {
    ...
    var base_buf: [512]u8 = undefined;
    const base = a.dupe(u8, @import("../util/fs.zig").testing.perPidDir(&base_buf, "cc-zig-lsp-sym")) catch return;
    defer a.free(base);
    mkdirZ(base);
    const gitdir = std.fmt.allocPrint(a, "{s}/.git", .{base}) catch return;
    defer a.free(gitdir);
