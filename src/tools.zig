...
pub const registry: []const ToolEntry = &.{
    ...
    .{
        .name = "BashOutput",
        .description = "A backgrounded job's exit is announced to you automatically; you do not need to call this in a loop. Read stdout/stderr and status of a backgrounded Bash job by job_id. Returns stdout/stderr chunks plus status (running|exited|killed). The call waits for new output or job exit when a running job has no unread requested bytes (default 30 seconds, maximum 600 seconds); set wait_ms to 0 for a snapshot. Omit stdout_since_byte/stderr_since_byte to continue from the last read, or pass an explicit offset to override it. Do not re-poll in a loop or sleep between polls: one BashOutput call waits for it. max_bytes caps a single read; omit it to get as much as the context budget allows (maximum 262144, and a large explicit value may still be reduced to fit that budget). To follow a long job, pass the previous stdout_next_offset/stderr_next_offset as the next stdout_since_byte/stderr_since_byte — those are the resume cursors. stdout_total_bytes/stderr_total_bytes report the file's current size, NOT a cursor: using them as one skips everything between what was shown and the end of the file. NOTE: if the command used shell redirection (>/>>/2>), the redirected output went to the file you named and BashOutput will not show it — Read that file directly.",
        .input_schema = .{ .type = "object", .prop_specs = &.{
            ...
            .{ .name = "wait_ms", .type = "integer", .description = "Wait for new output or exit when no unread requested bytes exist (default 30000 ms, 0 for an immediate snapshot, maximum 600000 ms)" },
    ...
    .{
        .name = "ReadArtifact",
        .description = "Read a bounded byte range from a recoverable tool-result artifact. Use this whenever a tool result reports it was truncated and names an artifact_id — re-running the tool costs a full round-trip and still will not return the omitted bytes. offset is zero-based bytes; limit is capped at 32768 and may be reduced further to fit the context budget, in which case next_offset carries the remainder. The result is always bounded and never spills recursively. A turn has a recovery allowance of half its result budget; calls beyond it return recovery_allowance_exhausted with the offset to resume from in the next turn.",
        .input_schema = .{ .type = "object", .prop_specs = &.{
    ...
    .{
        .name = "Monitor",
        .description = "Start a background monitor that runs a command and streams stdout line-by-line. Its exit is announced to you automatically. Use to watch logs, poll status, or react to file changes mid-conversation. Each stdout line is collected into a ring buffer readable via BashOutput(job_id). Stop with KillShell(job_id). Args: command (required), description (required, e.g. 'errors in /var/log/app.log'). Optional: persistent (default false; if true, no timeout).",
        .input_schema = .{ .type = "object", .prop_specs = &.{
