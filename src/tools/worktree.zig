...
pub fn enterExecute(ctx: *const ToolContext, args: []const u8) anyerror![]u8 {
    ...
    const out = common.spawnCaptureWithStderrTimed(argv_args[0..], a, ctx.abort, 30_000, ctx.spawn_tick_fn) catch |err| {
...
pub fn exitExecute(ctx: *const ToolContext, args: []const u8) anyerror![]u8 {
    ...
    if (std.mem.eql(u8, action, "remove")) {
        ...
        const out = common.spawnCaptureWithStderrTimed(argv_list.items, a, ctx.abort, 30_000, ctx.spawn_tick_fn) catch null;
        if (out) |o| {
