...
pub const jev_question = @import("jev/question.zig"); // typed System-One questions (Jev-Mem control plane)
pub const jev_client = @import("jev/client.zig"); // deadline-bounded System-One transport with breaker
pub const jev_advisor = @import("jev/advisor.zig"); // kernel-owned shadow/advisory decision seam
pub const jev_runtime = @import("jev/runtime.zig"); // METACODES_JEV_* wiring, address-stable runtime
pub const jev_excerpt = @import("jev/excerpt.zig"); // query-focused judge excerpts
...
pub const kgd_identity = @import("kg/kgd/identity.zig"); // what the local TinyKG service is, and to whom
pub const kgd_bridge = @import("kg/kgd/bridge.zig"); // one tinykgd child over NDJSON stdio
pub const kgd_server = @import("kg/kgd/server.zig"); // the authenticated HTTP contract clients speak
