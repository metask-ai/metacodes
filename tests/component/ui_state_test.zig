...
test "dispatch: 空 editor 按 ? → overlay=help" {
    var s = UiState{};
...
test "dispatch: help 下任意键关闭" {
    var s = UiState{ .overlay = .help };
...
test "dispatch: phase_change → generating 重置 spinner + 关 overlay" {
    var s = UiState{ .overlay = .help, .spinner = .{ .frame = 9 } };
