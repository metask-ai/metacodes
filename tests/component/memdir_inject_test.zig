...
test "L2: memdir 启用 → system prompt 含 # Memory 段 + memdir 路径(进请求体)" {
    ...
    try std.testing.expect(std.mem.indexOf(u8, sys, "knowledge graph") == null);
    try std.testing.expect(std.mem.indexOf(u8, sys, "KgRemember") == null);
}

test "L2: TinyKG-ready memory prompt advertises Markdown graph projection" {
    const a = std.testing.allocator;
    const sp = try cc.system_prompt.buildFull(
        a,
    ...
    try std.testing.expect(std.mem.indexOf(u8, sp, "KgRemember") != null);
    try std.testing.expect(std.mem.indexOf(u8, sp, "/kg sync") != null);
}
