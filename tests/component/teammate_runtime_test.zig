...
test "L2 teammate MAX_TEAMMATES 上限强制执行 + working 期 is_active=true" {
    ...
    const bodies = [_][]const u8{TURN1_SSE};
    var srv = try harness.MockServer.startCassette(&bodies, 5000); // 慢:保证全体 working
    defer srv.stop();
    const url = try srv.urlOwned(a);
    defer a.free(url);
...
test "L2 teammate 重名拒绝 + 上限存在" {
    ...
    const bodies = [_][]const u8{TURN1_SSE};
    var srv = try harness.MockServer.startCassette(&bodies, 3000); // 慢:保证首个 teammate 保持 working
    defer srv.stop();
    const url = try srv.urlOwned(a);
    defer a.free(url);
