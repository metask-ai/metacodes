...
const RejectDispatchStartSink = struct {
    ...
    fn emit(raw: *anyopaque, event: cc.tools.tool_observation.Event) bool {
        const self: *@This() = @ptrCast(@alignCast(raw));
        return switch (event) {
            .rule_filter, .rule_coverage_gap, .rule_bounds_overflow, .verification_final_gate, .requirement_ledger, .test_weakening_candidate, .formal_decision, .formal_decision_batch => blk: {
