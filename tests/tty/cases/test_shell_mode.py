...
空框打 `!` → 前缀 ❯→!、placeholder→`Try "fix lint errors"`、footer→`! for bash mode`;
...
def _footer_row(sc):
    for r in range(sc.rows):
        t = sc.line_text(r)
        if "bash mode" in t or "shift+tab to cycle" in t:
            return r, t.strip()
    return None, ""


def test_shell_enter_on_bang(bin_path):
    # 空框打 `!` → 前缀 `!` + placeholder + footer `! for bash mode`。
    ...
    assert "! for bash mode" in ftext, f"footer 应为 '! for bash mode',实得 {ftext!r}"
