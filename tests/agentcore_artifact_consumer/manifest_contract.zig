...
pub fn validateManifest(manifest: Manifest, expected: Expected) Error!void {
    ...
    if (manifest.contract.binary_abi_version != 1 or
        manifest.contract.binary_abi_revision != 9 or
        manifest.contract.binary_abi_table_size != 280 or
        manifest.contract.capabilities != 0x3fffff)
...
fn validManifest() Manifest {
    return .{
        ...
        .contract = .{
            ...
            .binary_abi_revision = 9,
            .binary_abi_table_size = 280,
            .capabilities = 0x3fffff,
        },
        .files = &valid_files,
    };
}
...
test "manifest contract rejects toolchain target optimize and ABI drift" {
    ...
    manifest = validManifest();
    manifest.contract.binary_abi_revision = 6;
    try std.testing.expectError(error.AbiMismatch, validateManifest(manifest, valid_expected));
    manifest = validManifest();
